...
- **Cd 的参考面积和积分范围**：我把标准配置的车身和 −y 侧车轮在地面以上的部分投影到 y-z 平面，测得半车迎风面积约 1.37 m²，比 `Aref 1.60` 小约 14%。如果 Aref 本意是半车迎风面积，那么输出的 Cd 会偏低约 14%。另外，forces / forceCoeffs 的 `patches` 只包含车身，**不含车轮**（`FR_mm`、`RR_mm`），所以原文 Cd 里没有车轮阻力。
