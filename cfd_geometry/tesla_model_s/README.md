...
> ⚠️ `system/forceCoeffs` 里 `Aref 2.2` 对应整车迎风面积（按 `baseline.stl` 投影实测约 2.36 m²），但计算域只有半车，力也只有一半，所以输出的 Cd/Cl 约为真实值的一半。要么把 Aref 改成半车面积（原作者估计值为 1.1，实测约 1.18 m²），要么把结果乘 2。
